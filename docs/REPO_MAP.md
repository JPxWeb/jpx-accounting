...
**Verified against code:** 2026-08-09 (commit `b59ed2d` + working tree). Line numbers drift — treat them as anchors, not gospel. Update this file when adding routes, events, packages, or storage keys.
...
| `/books`                 | `components/screens/books-screen.tsx` → `books/{journal,general-ledger,trial-balance,suppliers,close}-view.tsx`; journal URL state: `?ledgerMode=inline\|drawer`, `?voucher=<id>`, `?q=<search>` | `GET /api/reports/journal`, `…/trial-balance`, `["workspace"]`                                                                                                                        |
...
Cross-cutting: `components/app-shell.tsx` (nav + mobile dock + capture sheet), `components/command-palette.tsx` (Cmd-K), `components/period/period-selector.tsx` + `period-options.ts`, `hooks/use-period-scope.ts` (resolves `?period=` through the **domain** `resolvePeriodToken` so client and server can't disagree). The Books journal keeps its overview mode (`?ledgerMode=inline|drawer`), selected voucher (`?voucher=<id>`), and search (`?q=<search>`) in URL state; `journal-view.tsx` owns that grammar.
...
| `docs/README.md`                 | Authority index — which doc wins for invariants / how-to / status / navigation                                                                                                                                                                                                                                          | 2026-08-06          |
| `docs/architecture.md`           | Runtime shape                                                                                                                                                                                                                                                                                                           | verified 2026-08-06 |
| `docs/DEV_STATUS.md`             | Phase status + open follow-ups                                                                                                                                                                                                                                                                                          | reviewed 2026-08-06 |
...
| `docs/superpowers/plans/`        | **Active** plans only after F-3 archive. Keep: consolidation backlog (`2026-08-06-repo-health-consolidation-plan.md`), strategy baseline (`2026-07-18-full-sweep-next-steps-plan.md`), product bets (`2026-07-19-opportunity-backlog.md`). Landed Wave A–C + advisory/Track-A plans → `docs/archive/superpowers/plans/` | per-file            |
| `docs/superpowers/specs/`        | 7 design specs; governing: `2026-07-03-advisory-pivot-design.md` (Approved)                                                                                                                                                                                                                                             | per-file            |
| `docs/superpowers/*.md`          | Session handovers (Aug-6 repo-health handover STATUS-updated post Waves A–C)                                                                                                                                                                                                                                            | per-file            |
