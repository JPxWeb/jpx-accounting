...
The 2026-05-13 IA spec defines the *product design* for the 5-tab IA. Phases 1–4 shipped (5-tab nav, digest, redirects, Books drill-through, Today keyboard flow). Phases 5–8 remain:
...
This document does **not** re-do product design. It is an **execution delta**: it re-baselines Phases 5–8 against the *current* tree, records the decisions taken on 2026-05-19, and resolves the points where the parent spec or the old plan diverged from code reality.
...
- `profit-loss.ts` — Resultaträkning, *kostnadsslagsindelad*. BAS rollup:
...
**Field-persisted settings (mirror the `companySettings` pattern — `store.ts:588-595`, *not* event-sourced):**
