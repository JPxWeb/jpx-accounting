...
**Purpose:** Single home for all evidence — drafts in progress, freshly captured, fully archived. Adding new evidence happens from here _and_ via the global FAB (which stays).
...
**Purpose:** Inspect the ledger. This is where accountants live. It's separate from Reports because Reports are _outputs_ (statutory documents), while Books is _exploration_ (drill through accounts).
...
**Purpose:** Statutory and management _outputs_. Reports are read for filing, board meetings, and accountant handoff — not for daily exploration (that's Books).
...
- Phase 2 graduated AI autonomy (the AI-posture _page_ exists, but actual auto-approve logic is later)
- Mobile camera capture wiring (the _page_ exists; OCR pipeline is separate)
