...
**N1 · Push, merge, and clean the repo** — _Why:_ R11 is the highest operational risk (deploy fixes exist on one disk; origin runs broken config). _Scope:_ push local `main`; publish `fix/verified-sweep-fixes` with the two-line doc corrections (CLAUDE.md:167, docs/architecture.md:79 still claim RS256-only); PR + merge; delete `feat/advisory-pivot` locally and on origin (tree-verified identical to the PR #30 squash); delete `api-deploy-test/` (4.6 MB reproducible build artifact) and add `api-deploy*` to `.gitignore`; commit the `.serena/project.yml` schema migration; fix the DEV*STATUS.md U+FFFD mojibake (lines 115-121) and the self-contradicting extraction follow-up bullet (line 86 vs line 23); commit-or-push sweep of the `brain/` repo (unpushed branch + dirty note). \_Effort:* **S**. _Accept:_ `git status` clean in both repos; origin/main contains all 4 commits; `git branch -a` shows no feat/advisory-pivot.
