...
test("SKU movement list route returns exact derived JSON without appending", async () => {
  const store = new MemoryLedgerStore();
  store.getEvents = async () => [];

  const response = await appWith(store).request("http://localhost/api/lists/sku-movements");
...
test("SKU movement list clients validate HTTP and offline rows", async (t) => {
  const store = new MemoryLedgerStore();
  store.getEvents = async () => [];
  ...
  assert.deepEqual(await httpClient.getSkuMovementsList(), []);
  assert.deepEqual(await offlineClient.getSkuMovementsList(), []);
});
