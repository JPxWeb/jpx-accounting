...
test("conformance registry includes enrichment confirmation safety", () => {
  ...
  assert.ok(
    CONFORMANCE_SCENARIOS.some((scenario) => scenario.name === "invoice review approval registers once atomically"),
  );
  assert.ok(
  ...
  assert.ok(
    CONFORMANCE_SCENARIOS.some(
      (scenario) => scenario.name === "quantity inventory review approval records once atomically",
    ),
  );
});
